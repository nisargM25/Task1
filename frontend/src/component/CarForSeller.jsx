...
const CarForSeller = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const res = await axios.get(`http://10.0.3.98:9000/api/cars/userS?page=${page}&limit=${limit}`, { headers: { authorization: `Bearer ${currentUser.accessToken}` } });
                        ...
                        <div className="row">
                            {
                                cars.length > 0 ? (
                                    ...
                                    <div>
                                        <h1 className='text-center'> No Car For Sale</h1>
                                    </div>
                                )
                            }
                        </div>
